...
* Liberal license (`BSD-3-Clause`, `Apache-2.0`, plus various other open-source licenses for dependencies of PDFium.)
...
[^2]: Other rare operating systems / CPUs exist, such as `FreeBSD`, `Illumos (OpenIndiana/OmniOS)`, `AIX`, `IBM z/OS`, `Haiku`, `SerenityOS` / `SPARC`, `Elbrus 2000`, plus big-endian variants of some CPUs (e.g. `PowerPC`, `MIPS`), ... but it is not feasible to provide pre-builds for any arbitrary platform under the roof of this project. To do so, we need at least one of the following:
